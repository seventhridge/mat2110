...
def gameloop():
    if t1.xcor() >= 190 and t1.xcor() >= 190:
        flyingturtlemove()
        enemyturtlemove()

    if t1.xcor() > -200 and t1.xcor() < 190:
        flyingturtlemove()
        enemyturtlemove()
    if t1.xcor() <= -200 and t2.xcor() <= -200:
...
# main loop of the gam  e

print("Press the space bar to jump up through each gate!")
wn.listen()
